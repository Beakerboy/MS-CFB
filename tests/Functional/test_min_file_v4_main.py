...
def test_min_file(mocker):
    ...
    main()
    assert os.stat("Testv4.bin").st_size == 4096 * 3

    f = open("Testv4.bin", "rb")
    ...
    sector3 = bytes.fromhex(" ".join(root)) + unused * 31
    assert f.read(4096) == sector3
